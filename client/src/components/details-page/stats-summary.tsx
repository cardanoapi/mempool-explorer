...
export default function StatsSummary() {
    ...
    return (
        <div className={'flex flex-col items-center gap-2'}>
            {stats?.map((stat, idx) => (
                <div key={idx} className={'flex flex-col gap-2 border-b-[1px] items-start justify-center border-b-gray-300 last:border-none py-3 first:pt-0 last:pb-0'}>
                    <div className={`flex gap-2`}>
                        ...
                        <div className={`flex gap-2`}>
                            {Object.entries(stat).map(
                                ([key, value]: any) =>
                                    key !== 'epoch' && (
                                        <div key={key} className={`${key === 'epoch' ? '' : 'border-solid border-[1px] bg-blue-50 px-3 py-1 border-blue-800'}`}>
