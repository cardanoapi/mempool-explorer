...
import { consumer as consumerMaker, kafkaClient, blockConsumer } from './kafkaconsumer';
...
offset.fetchLatestOffsets([process.env.KAFKA_TOPIC_NAME],(er,mempoolOffset)=>{
    offset.fetchLatestOffsets([process.env.KAFKA_BLOCK_TOPIC_NAME],(err,blockoffset)=>{
        if(er | err){
            console.log("Error fetching offsets. Make sure the topics exist in the kafka cluster");
            return;
        }
        console.log(mempoolOffset, blockoffset);
        const kafkaConsumer = consumerMaker(mempoolOffset[process.env.KAFKA_TOPIC_NAME]['0'], blockoffset[process.env.KAFKA_BLOCK_TOPIC_NAME]['0']);
                ...
                if(cb[0]==0){
                    ...
                    if(msg.value.length != 0){
                        txHashes = cbor.decodeFirstSync(msg.value).map((tx:Buffer)=>{  
                            const hash =  blake2b.createHash('blake2b',{digestLength:32}).update(cbor.encode((cbor.decodeFirstSync(tx))[0])).digest();
                            return hash;
                        });
                    }
